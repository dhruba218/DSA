...
public class FetchRecordsFromCSV {
	...
	// Uncomment the method getPermanentEmployee
	 ...
	 public static PermanentEmployee[] getPermanentEmployee(String fileName) throws IOException {
		ArrayList<ExcelList> empListPermanent = readFromCSV(fileName);
	  	if(empListPermanent.size() == 0) {
	  		System.out.println("The data in the PermanentEmployeeRecords.csv is missing");
	  		System.exit(0);
	  	}
	  	PermanentEmployee[] perm = new PermanentEmployee[empListPermanent.size()];
	  	int i = 0;
	  	for (ExcelList e : empListPermanent) { //e.getExtraField()
	  		perm[i] = new PermanentEmployee(e.getEmpId(), e.getName(), e.getAge(), e.getContact(), e.getCompensation(), e.getRating(),e.getExtraField() );
	  		i++;
	  	}
	...
	// Uncomment the method getContractEmployee
	 ...
	 public static ContractEmployee[] getContractEmployee(String fileName) throws IOException {
	 	...
	 	int i = 0;
	 		...
	 		for (ExcelList e : empListContract) {  //e.getExtraField()
	 			contract[i] = new ContractEmployee(e.getEmpId(), e.getName(), e.getAge(), e.getContact(), e.getCompensation(), e.getRating(),e.getExtraField());
	 			i++;
	 		}
	 		return contract;
	 	}
